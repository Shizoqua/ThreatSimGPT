...
CLI commands for the Ciicerone VM MCP Server.
"""
...
def start(config, log_level, host, token):
    ...
    console.print(Panel.fit(
        "[bold green]Ciicerone VM MCP Server[/bold green]\n"
    ...
    try:
        from ciicerone.mcp import run_server
...
def tools():
    ...
    """
    try:
        from ciicerone.mcp.tools import ALL_TOOLS
    except ImportError:
    ...
    console.print(Panel.fit(
        f"[bold]Ciicerone MCP Server Tools[/bold]\n"
...
def status():
    ...
    settings = [
        ("Server Name", os.getenv("CIICERONE_SERVER_NAME", "ciicerone-vm")),
...
def export_schema(output):
    ...
    schema = {
        "name": "ciicerone-vm",
        "version": "0.1.0",
        "description": "Ciicerone VM MCP Server - AI-driven attack simulations",
        "tools": []
    }
