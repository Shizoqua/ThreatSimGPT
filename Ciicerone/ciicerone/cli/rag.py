...
def rag_status():
    ...
    if not data_dir.exists():
        ...
        click.echo("   Run 'ciicerone rag init' to initialize")
    else:
...
def rag_init(config: Optional[str], force: bool):
    ...
    if not config_path.exists() or force:
        default_config = """# Ciicerone RAG Configuration
# ================================
...
vectorstore:
  ...
  port: 7687
  database: "neo4j"
  collection_name: "ciicerone_intelligence"
...
"""
    ...
    click.echo("     ciicerone rag ingest --source mitre")
    click.echo("")
    click.echo("  4. Generate a playbook:")
    click.echo("     ciicerone rag generate --scenario 'phishing attack'")
...
async def _ingest_source(source: str):
    ...
    from ciicerone.rag import IntelligenceIngester, RAGConfig
...
async def _search_intelligence(query: str, top_k: int, source: Optional[str]):
    ...
    from ciicerone.rag import VectorStoreManager, HybridRetriever, RAGConfig
    from ciicerone.rag.config import EmbeddingConfig, VectorStoreConfig, RetrieverConfig
...
async def _get_stats():
    ...
    from ciicerone.rag import VectorStoreManager, RAGConfig
