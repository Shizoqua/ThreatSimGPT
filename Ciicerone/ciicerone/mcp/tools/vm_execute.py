...
async def execute_ssh_command(
    ...
    command: str,
    username: str = "root",
    password: str = "ciicerone",  # nosec B107 - Intentional lab default
    timeout: int = 60
) -> dict:
    """
...
async def handle_vm_execute(
    ...
    if name == "vm_execute":
        ...
        username = os.getenv("CIICERONE_VM_USERNAME", "root")
        password = os.getenv("CIICERONE_VM_PASSWORD", "")
        ...
        if not password:
            return [TextContent(
                ...
                text=json.dumps({
                    ...
                    "reason": "Set CIICERONE_VM_PASSWORD environment variable"
                }, indent=2)
            )]
        ...
        script_name = f"/tmp/ciicerone_script_{vm_id}{conf['ext']}"

        # Upload script
        upload_cmd = f"cat > {script_name} << 'CIICERONE_EOF'\n{script}\nCIICERONE_EOF"
